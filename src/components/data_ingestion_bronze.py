...
from src.utils import read_yaml, convert_to_datetime
...
class BronzeIngestion:
    ...
    def prepare_disbursal_date(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df = convert_to_datetime(df, 'DisbursalDate', format='%d-%m-%y')
    ...
    def save_bronze_partitions(self, df: pd.DataFrame) -> list:
        '''
        ...
        try:
            ...
            for (year, month), df in df.groupby(['year', 'month']):
                ...
                logging.info(f"Saved bronze partition {year}-{month:02d}: {len(partition_df)} records to {partition_path}")
    ...
    def initiate_bronze_ingestion(self) -> list:
        ...
        try:
            ...
            saved_paths = self.save_bronze_partitions(df)
...
if __name__ == "__main__":
    ...
    saved_paths = bronze_ingestion.initiate_bronze_ingestion()
    print(f"Bronze partitions saved to: {saved_paths}")